...
$(function(){
	...
	$('.filter li a').mousedown(function(){
		...
		var brandId = "";
		var paraList = "";
		//每当点击一个筛选条件的时候，就要遍历整个筛选条件获取所有被选中的a链接的值
		$('.filter li a').each(function(){
			//如果a连接上有class="here"说明是被选中的
			var clazz = $(this).attr("class");
			if(clazz == "here"){
				//在筛选条件中每一个a链接都加上两个自定义的属性，一个是fType用于区分筛选条件的类型
				//一个是fValue用于存储筛选条件的值
...
<div class="bar"><div class="bar_w">
	<p class="l">
		...
		<span class="l">
			欢迎来到中国移动！<a href="javascript:void(0);" title="登录" id="loginAlertIs" class="orange"><samp>[</samp>请登录<samp>]</samp>
		...
		<h2 class="h2 h2_filter mt"><em title="手机商品筛选">手机&nbsp;-&nbsp;商品筛选</em><cite><a href="javascript:void(0);" id="filterRest" title="重置筛选条件">重置筛选条件</a></cite></h2>
			...
			<ul class="uls filter">
				...
				<li><b>价格：</b><p>
					...
					<a href="javascript:void(0);" fType="price" fValue="1-99">1-499</a>
				...
				<li><b>品牌：</b><p>
					<a href="javascript:void(0);" fType="brand" fValue="" title="不限" class="here">不限</a>
					<c:forEach items="${bList }" var="brand">
						<a href="javascript:void(0);" fType="brand" fValue="${brand.brandId }">
							${brand.brandName }
						</a>
					</c:forEach>
				</p></li>
