	...
	$.ajax({
		...
		success:function(responseText){
			...
			if(selectId == "#mycity"){
				...
				//如果选择城市，加载城市下的区县，只清区县下的option即可
				$(selectId).empty();
...
function modifyDefaultAddr(shipAddrId){
	window.location.href="${path}/user/login/modifyDefaultAddr.do?shipAddrId="+shipAddrId;
	/* $.ajax({
		url:"${path}/user/login/modifyDefaultAddr.do",
		type:"post",
		dataType:"text",
		data:{
			shipAddrId:shipAddrId
			...
			<div class="ofc">
					...
					<c:forEach items="${addrList }" var = "addr">
						<tr <c:if test="${addr.defaultAddr == 1 }">class='here'</c:if>>
							...
							<c:if test="${addr.defaultAddr == 1 }">
								<td class="def blue" >默认地址&nbsp;&nbsp;</td>
							</c:if>
							<c:if test="${addr.defaultAddr == 0 }">
								<td class="def blue" >
								<a href="javascript:void(0);" title="设为默认" onclick="modifyDefaultAddr(${addr.shipAddrId})" >[设为默认]</a>
								</td>
								<!-- <td class="def blue" >设为默认</td> -->
							</c:if>
							<td>
								<a href="javascript:void(0);" title="修改" onclick="modify(${addr.shipAddrId})" class="blue">[修改]</a>
								<a href="javascript:void(0);" title="删除" onclick="del(this)" class="blue">[删除]</a>
							</td>                  
						</tr>
					</c:forEach>	
				</tbody>
				...
				<form id="jvForm" action="${path }/user/login/saveOrUpdateAddr.do" method="post">
					...
					<li>
						...
						<span><input type="checkbox" id="defaultAddr" name="defaultAddr" value="1"/>设为默认收货地址</span>
					</li>
