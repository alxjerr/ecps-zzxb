		...
		<dd>
			...
			<div class="box_d bg_gray2 ofc">
				<table cellspacing="0" summary="" id="adrList" class="tab">
					...
					<tr>
						<td colspan="4">
							<input type="radio" <c:if test="${addr.defaultAddr == 1 }">checked</c:if> value="${addr.shipAddrId }" name="address" />
								${addr.provText }&nbsp;${addr.cityText }&nbsp;${addr.distText }&nbsp;
								${addr.addr }&nbsp;${addr.zipCode }&nbsp;${addr.phone }&nbsp;${addr.shipName}								
						</td>
					</tr>
					</c:forEach>
					<tr>
						<td colspan="4">
							<input type="radio" value="add" name="address" />新增收货地址
						</td>
					</tr>
				</table>
			</div>
